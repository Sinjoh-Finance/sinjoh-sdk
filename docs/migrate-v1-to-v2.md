...
to `2.2.1`. The API keeps its `/v1` URL; the package major changes because wallet execution and
...
| --- | --- | --- |
| Contracts and deployments | The production manifest now includes Project Launcher V2, its Registry, launch validator, launchpad adapters, governed modules, and the complete routed-asset implementation. Routes can send directly, swap, buy back and burn, fund Treasury/Airdrop/Raffle/liquidity/project sinks, and normalize into another route. | Read addresses and integration proofs from `@sinjoh/deployments`; do not retain copied v1 addresses or hand-build proofs. Run `verifyManifest` or `createSinjohClient().verify()` before use. |
...
| ABIs | `@sinjoh/abis` adds the Project V2 Launcher, validator, Registry, Router, Treasury, staking, Airdrop, Raffle, and remaining module ABIs. The Router ABI contains the final action tuple shapes. | Recompile generated calls against the v2 ABI types. Import generated ABI values such as `projectLauncherV2Abi`, `projectRegistryV2Abi`, and `projectRouterV2Abi`; do not retain a locally copied ABI. |
| SDK | `@sinjoh/sdk` adds reviewed Project presets, deterministic prediction, complete route encoders, existing-token launches, Project Registry reads, provenance manifests, and exact Pons Project launch assembly/simulation/verification. | Build creator input with `buildLaunchFromPreset` or `buildExistingTokenLaunchFromPreset`, validate it against the packaged Launcher, then assemble and simulate the exact launch transaction before signing. |
...
- [ ] All installed `@sinjoh/*` packages use the same `2.2.1` release line.
