...
/**
 * A router with WETH + subject intake, one WETH-output bucket with a guarded subject->WETH
...
function stubClient(overrides: Record<string, unknown> = {}): PublicClient {
  const routes: Record<string, unknown> = {
    ...
    "bucketCount": 1,
    "bucketInfo:[0]": [{ kind: 1, token: WETH }, WETH, 10000, 1n, 2n],
    "conversionInfo:[0,0]": [{ kind: 2, token: ZERO }, SUBJECT, GUARD, GUARD, "0x", 60n, 0],
    ...
    [`bucketInputOwed:[0,"${SUBJECT}"]`]: 90n,
    [`protocolOwed:["${WETH}"]`]: 7n,
    [`protocolOwed:["${SUBJECT}"]`]: 0n,
    "allocationKey:[0,1]": 1,
    [`sinkOwed:[1,"${WETH}"]`]: 30n,
    [`walletOwed:["${WALLET}","${WETH}"]`]: 70n,
    ...overrides
  };
  return {
...
test("plans the full routing lifecycle from live state", async () => {
  ...
  assert.equal(subjectSync.call.args.length, 2, "guarded sync carries a caller floor");

  const conversion = byKind.get(`process-bucket:${SUBJECT}`);
  ...
  const wallet = byKind.get(`send-wallet:${WETH}`);
  assert.ok(wallet);
  assert.equal(wallet.amount, 70n);
  assert.deepEqual(wallet.call.args, [WALLET, WETH, 70n]);

  const sink = byKind.get(`fund-sink:${WETH}`);
  assert.ok(sink);
...
test("a quiet router plans nothing", async () => {
  const plan = await planRouterWork(stubClient({
    ...
    [`bucketInputOwed:[0,"${SUBJECT}"]`]: 0n,
    [`protocolOwed:["${WETH}"]`]: 0n,
    [`sinkOwed:[1,"${WETH}"]`]: 0n,
    [`walletOwed:["${WALLET}","${WETH}"]`]: 0n
  }), ROUTER);
...
test("an uncapped conversion takes the full owed amount", async () => {
  const plan = await planRouterWork(stubClient({
    "conversionInfo:[0,0]": [{ kind: 2, token: ZERO }, SUBJECT, GUARD, ZERO, "0x", 0n, 0]
  }), ROUTER);
